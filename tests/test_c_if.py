...
def test_single_clbit_invalid_variations(matrix) -> None:
    circuit = QuantumCircuit(
        2, 0, name=f"test_single_clbit_invalid_variations-{matrix[0]}-{matrix[1]}"
    )
    cr = ClassicalRegister(2, "creg")
    circuit.add_register(cr)
    circuit.measure(0, 0)
...
def test_single_register_index_invalid_variations(matrix) -> None:
    circuit = QuantumCircuit(
        2,
        0,
        name=f"test_single_register_index_invalid_variations-{matrix[0]}-{matrix[1]}",
    )
    cr = ClassicalRegister(2, "creg")
    circuit.add_register(cr)
    circuit.measure(0, 0)
...
def test_single_register_invalid_variations(matrix) -> None:
    circuit = QuantumCircuit(
        2, 0, name=f"test_single_register_invalid_variations-{matrix[0]}-{matrix[1]}"
    )
    cr = ClassicalRegister(2, "creg")
    circuit.add_register(cr)
    circuit.measure(0, 0)
...
two_bit_variations = [
    [
        [False, False],
        False,
        ["m qubit[0] => out[0]", "m qubit[1] => out[1]", "m qubit[2] => out[2]"],
    ],  # 00 => 00 (0)
    [
        [False, False],
        0,
...
def test_two_bit_register_variations(matrix) -> None:
    circuit = QuantumCircuit(
        3,
        0,
        name=f"test_two_bit_register_variations-{matrix[0][0]}-{matrix[0][1]}-{matrix[1]}",
    )
...
def test_two_bit_register_invalid_variations(matrix) -> None:
    circuit = QuantumCircuit(
        3,
        0,
        name=f"test_two_bit_register_invalid_variations-{matrix[0][0]}-{matrix[0][1]}-{matrix[1]}",
    )
...
def _eval(
    ir: bytes, gates: GateSet, result_stream: Optional[List[bool]] = None
) -> None:
